...
class Student(db.Model):
    ...
    department = db.relationship('Department', backref='department_students', uselist=False)#Provide it via the course department, one student belongs to only one department, one department has many students, 
...
#password and username for student login
#Create the model to handle department data and set a relationship between courses and a department

# Create a table for the results obtained in the course modules per semister
#Create a model object to handle Faculty data, include all departments in the faculty table
#Implement a one to many ie one Faculty has many departments